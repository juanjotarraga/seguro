...
import org.seguro.model.UsuarioRol;
...
public class UsuarioBean implements Serializable
{
   ...
   public String delete()
   {
      ...
      try
      {
         ...
         Aseguradora aseguradora = deletableEntity.getAseguradora();
         aseguradora.getUsuarios().remove(deletableEntity);
         deletableEntity.setAseguradora(null);
         this.entityManager.merge(aseguradora);
         Iterator<UsuarioRol> iterUsuarioRols = deletableEntity.getUsuarioRols().iterator();
         for (; iterUsuarioRols.hasNext();)
         {
            UsuarioRol nextInUsuarioRols = iterUsuarioRols.next();
            nextInUsuarioRols.setUsuario(null);
            iterUsuarioRols.remove();
            this.entityManager.merge(nextInUsuarioRols);
         }
   ...
   private Predicate[] getSearchPredicates(Root<Usuario> root)
   {
      ...
      Aseguradora aseguradora = this.example.getAseguradora();
      if (aseguradora != null)
      {
         predicatesList.add(builder.equal(root.get("aseguradora"), aseguradora));
      }
